...
const Login = () => {
  ...
  const HandleClick = (e) => {
    ...
    let isUser = creds?.users.filter((x) => {
      return x.email === email && x.password === password && x.role === "user";
    });
    let isAdmin = creds?.users.filter((x) => {
      return x.email === email && x.password === password && x.role === "admin";
    });
  ...
  useEffect(() => {
    ...
    if (!creds) {
      let obj = {
        users: [{ email: "admin@gmail.com", password: "123", role: "admin" }],
      };
